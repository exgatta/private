...
## 立体図をMinecraftらしく見せる方法

単色の立方体を並べても積み木にしか見えない。Minecraftらしさは主に2つから来る。

**1. テクスチャ（模様）**
16×16の升目に明るさを変えた長方形を置いたものを SVG の `<pattern>` にし、
アイソメの傾きに合わせて `patternTransform` で変形して面に敷く。

- **ブロックの形が小さくてもテクスチャを敷く**。パターンはマスの格子に合わせて
  敷き詰めてあるので、ハーフブロックには模様の下半分が、トラップドアには
  薄い一片がそのまま出る。マスいっぱいの箱にしか敷かないと、
  トラップドアも階段もハーフも単色の板きれになってしまう（実際にそうなった）
- **上面と側面で模様や色を変えられる**。草ブロック＝上が緑・側面が土、
  原木＝上が年輪・側面が樹皮。ここを分けないとMinecraftに見えない
  （パレットの `side_color` で側面の色を指定する）
- パターンは**ブロックの種類×3面（上・左・右）だけ**定義する。
  1ブロックずつ模様を描くと図形が何万個にもなってHTMLが破裂するが、
  この方法ならブロックが何千個あっても図形の数は変わらない
- タイルの寸法をちょうど1面ぶんにすると、ブロックの格子と模様が自然にそろう
- 模様の種類: 石＝ざらざら / 板材＝横の継ぎ目 / 石レンガ＝段違いの目地 /
  ガラス＝枠とハイライト / 葉＝まだら / 原木＝縦の樹皮 / 機械類＝枠とくぼみ

**2. ブロックの形**
Minecraftのブロックは立方体ばかりではない。立方体で代用すると別物に見える。
`blueprint/render.py` の `_iso_parts()` が種類ごとの形（箱のリスト）を返す。

| ブロック | 実物の形 | 立方体で描くとどう見えるか |
|---|---|---|
| レッドストーンダスト | 床に貼りついた線。**つながる向きへ腕が伸びる** | 赤い小箱が宙に浮く／板だけだと配線が追えない |
| レッドストーントーチ | 細い棒＋先端の火 | ただの赤い箱 |
| リピーター・コンパレーター | 薄い板＋小さな灯2つ | ただの白い箱 |
| ホッパー | 上が広く下が細い漏斗 | ただの黒い箱 |
| 感圧板 | 床の薄い板 | 浮いた箱 |
| ピストン・ディスペンサー | ふつうの立方体 | （小さく描くと逆に別物） |
| 階段 | 下半分＋奥半分の段差 | 屋根が板を重ねただけに見える |
| 花・ランタン・看板 | 細い棒＋頭 | 箱 |

`_iso_box()` は「立方体の一部」（位置のずれ・幅・奥行き・高さを個別指定）を
描く汎用の関数なので、この表の形はすべてその組み合わせで作れる。

- `blueprint/render.py` の `_iso_box()` が「立方体の一部」を描く汎用の関数。
  位置のずれ・幅・奥行き・高さを別々に指定できるので、階段以外の形にも使える
- 段の高い側は**矢印の向きの側**にしている。屋根なら矢印を棟へ向けると
  段が棟に向かって上がる（逆にすると屋根が谷に見える）

## 配線の「つながり」をどう見せるか

ダストを床の赤い板で描くだけでは、Minecraftらしくはなっても
**どこからどこへつながっているのかが読めない**（実際にそうなった）。
Minecraftはダストを「つながる向きへ線が伸びる形」で描いていて、
これが配線を追うための情報そのもの。

`_dust_links()` がつながり先を判定する。判定はMinecraftと同じ考え方:

- となりが**同じ高さのダストか回路部品**（トーチ・リピーター・コンパレーター・
  レバー・ピストン・ディスペンサー・ドロッパー・オブザーバー・
  レッドストーンブロック・感圧板）→ まっすぐつながる
- となりの**1段上**にダストがあり、自分の真上がふさがっていない → 登る
- となりの**1段下**にダストがあり、となりのマスがふさがっていない → 下る
- どこにもつながらない → 点1つ（Minecraftでは十字に見える）

これを立体図では「中心＋腕」の形で、上から見た図では「中心の四角＋線」で描く。
段差でつながるところには「上」「下」の文字を添える。
高さが変わる配線は現物でいちばん間違えやすいので、図で分かるようにしてある。
