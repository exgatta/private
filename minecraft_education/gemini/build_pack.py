...
def check_files():
    ...
    cmd = [
        ...
        "#",
        "# 【いちばん簡単な答え合わせ】ブロックの知識がなくても分かる自動判定:",
        "#  - 青い羊毛の列が「北」の目印（すべての「北向き」はこの列の方を向くのが正解）",
        "#  - 東の端に置かれる2台のピストンが「勝手に伸びて」いれば正解:",
        "#     1台目: 青い羊毛の方（北）へ頭が伸びる → ピストンの向きが正しい",
        "#     2台目: 頭が上へ飛び出す → リピーターの向き（矢印の先へ信号）が正しい",
        "#    伸びていない・別の方向へ伸びている場合は写真を送ってください",
        "",
        "def muki():",
        "    # 北の目印（青い羊毛の列）",
        '    player.execute("fill ~0 ~0 ~0 ~13 ~0 ~0 blue_wool")',
        "    # 自動判定1: 動力入りピストン。正しければ北（青い羊毛side）へ伸びる",
        '    player.execute("setblock ~13 ~0 ~17 redstone_block")',
        '    player.execute("setblock ~13 ~0 ~16 sticky_piston 3")  # 北向き',
        "    # 自動判定2: 動力→リピーター（北向き矢印）→ダスト→上向きピストン。",
        "    # リピーターが正しい向きなら信号が届き、ピストンの頭が上へ飛び出す",
        '    player.execute("setblock ~13 ~0 ~27 redstone_block")',
        '    player.execute("setblock ~13 ~0 ~26 unpowered_repeater 0")  # 北向き',
        '    player.execute("setblock ~13 ~0 ~25 redstone_wire")',
        '    player.execute("setblock ~13 ~0 ~24 sticky_piston 0")  # 上向き',
    ]
    z = 2
