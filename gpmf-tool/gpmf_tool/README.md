...
**結合すべきかの判定**は、カメラごとの命名規則と中身の両方で行う
(`split_detect.py`):

1. **絶対条件** — コーデック・解像度・トラック構成が一致しないものは
   決して結合しない (読めないファイル・`._` 付随ファイル・Insta360 の
   LRV プロキシは単独扱い)
2. **メーカー固有の命名規則** (確度: 高)
   - GoPro `GH010001.MP4` … 同一ファイル番号 + 連続チャプター
     (旧 `GOPR0001` + `GP010001` も対応)
   - DJI `DJI_20260914145635_0011_D.MP4` … 連番が連続 + 次のどちらか
     - **動画内の分割情報** (Osmo Pocket 4 Pro 等)。DJI は `moov/udta` に
       `fsid` (その撮影の **先頭ファイル** の SD カード上のパス、64 バイト)
       と `gpid` (撮影内の位置 2, 3, …。**続きのファイルにだけ** ある、
       4 バイト整数) を書く。先頭ファイルと単独のファイルは `gpid` を持たず
       `fsid` が自分自身を指す。カメラ自身の記録なので、これがあれば
       ファイル名や動画内の時刻と矛盾してもこちらに従う
       (実測: 29 ファイル中 6 組の分割すべてで `fsid` が先頭ファイル、
       `gpid` が 2→3→4… と連続していた)
     - それが無い機種は、ファイル名の日時が「前の開始 + 前の長さ + 3 秒」
       以内 (Osmo Pocket/Action はセグメントごとに開始時刻を書く。
       全部同じ時刻を書く機種も可。同じカメラで前の撮影が終わる前に別の
       撮影は始められないので、前の範囲内なら続き。次の時刻がそれより後
       = 間に停止があった = 別撮影)
     単独と判定された動画には「前/次の動画とは別撮影」と判断した理由が
     数値付きで表示される。「判定の詳細」の各ファイル行には
     `DJI 内部 fsid=… gpid=…` が出る
   - Insta360 `VID_20250915_125402_00_062.mp4` … 同一日時キー
3. **状況証拠が要るもの** — DJI 旧 `DJI_0001`、Sony `C0001`、一般的な
   連番 `xxx_001` は「新しい撮影でも番号が進む」ため、
   **撮影時刻の連続性** (次の開始 ≈ 前の開始 + 前の長さ、±120秒) か
   **分割サイズの揃い方** (末尾以外が 1GB 以上で 5% 以内) を要求する
4. 命名規則が不明でも撮影時刻が連続していれば結合候補にするが、
   確度「低 (要確認)」として表示する

撮影時刻が明らかに離れているなど矛盾があれば結合しない。
...
#### GUI: エクスプローラー風の選択画面 (ブラウザで開く Web UI)

GUI の **「フォルダから結合」** ボタン、または CLI の `gpmf ui [フォルダ]` で、
OS 既定のブラウザに結合選択画面が開く。Python 側は `127.0.0.1` の空きポートで
小さなサーバ (標準ライブラリのみ・外部 CDN なし) を立てているだけで、
外部からは到達できない (起動ごとのトークン必須)。

```bash
python -m gpmf_tool ui ~/Videos/DCIM      # フォルダを渡すとすぐ精査が始まる
python -m gpmf_tool ui --no-browser       # URL だけ表示 (手で開く)
```

```
 ☐ | サムネイル | 名前                        | サイズ  | 長さ | 解像度      | 撮影日時         | 種類
 ▼ ☑ 分割された1本の撮影 — 3個 / 12:04 / 5.31 GB  [確度 高]  撮影時刻が連続 …      3 ファイル
   ☑  [🖼]  VID_20250915_125402_00.mp4   1.73 GB   4:01   5760 x 2880   2025/09/15 12:54   360度
   ☑  [🖼]  VID_20250915_125803_00.mp4   1.79 GB   4:02   5760 x 2880   2025/09/15 12:58   360度
   ☐  [🖼]  VID_20250915_130205_00.mp4   1.79 GB   4:01   5760 x 2880   2025/09/15 13:02   360度
```

- **Windows エクスプローラーの「詳細」表示風** — 固定ヘッダ、列ヘッダのクリックで
  並べ替え、境界のドラッグで列幅変更、縞模様・ホバー・選択ハイライト、
  「グループで表示」のようなグループ見出し行 (判定理由・確度・合計)
- **サムネイル** — 埋め込みサムネ → 同名 `.THM`/`.jpg`/`.png` → ffmpeg の順に取得。
  無ければブラウザが動画の 1 コマを描き (対応コーデックのとき)、それも無理なら
  解像度/長さのプレースホルダ。表示サイズは 小 / 中 / 大
- **取捨選択** — チェック欄 / Space キー / グループ見出しのチェックでまとめて切替。
  Ctrl(⌘)+クリックで複数選択、Shift+クリックで範囲選択、Ctrl(⌘)+A で全選択、
  Enter でグループの折りたたみ
- **詳細ペイン** — フルパス・コーデック・fps などと、読めないファイルの理由。
  グループを選ぶと結合される順番
- **ステータスバー** — 動画数 / 結合対象数 / 選択中の合計サイズ (必要な空き容量)
- **フォルダ選択は OS のダイアログ** (macOS: osascript / Windows: PowerShell /
  Linux: zenity・kdialog)。使えない環境ではパスを直接入力
- **「結合する」** / **「GoPro 化 ▾」**(機種・GPX・内蔵GPS) で実行し、
  進捗ログと出来上がったファイルの一覧を表示。「フォルダを開く」で出力先へ、
  各結果の「ダウンロード」でブラウザからも保存できる
- **単独の動画も GoPro 化できる** — 「単独の動画も表示」で表示してチェックすると、
  「GoPro 化 ▾」が分割グループは結合してから、単独の動画は結合せずそのまま
  GoPro 化する (`<元の名前>_gopro.mp4`)。詳細ペインの
  「この動画だけ GoPro 化」で 1 本だけ処理することも、「ダウンロード」で元の
  動画をそのままブラウザに保存することもできる。非表示の単独動画は対象にしない
...
**撮影日時の表示 (一覧・詳細・結合結果・`info`)** はカメラの時計の値
(ローカル時刻) で出す。ファイル名に時刻を書く DJI
(`DJI_20260914145635_…`) と Insta360 (`VID_20250915_125402_…`) はその値を
優先する。Osmo Pocket 4 Pro は動画内の mvhd に **UTC** を書くため、動画内の
値をそのまま出すと日本では 9 時間早くなる (実測: 名前 13:04:11 に対し
動画内 04:04:12)。ファイル名が無い機種は動画内の値をタイムゾーン変換せずに
出す。「判定の詳細」の各ファイル行には動画内の値と名前の時刻との差
(`名前の時刻との差 -8:59:59 = UTC 記録`) も併記する。
結合結果のファイル内 (mvhd) の値は先頭素材のまま (DJI なら UTC のまま) で、
書き換えはしていない。
