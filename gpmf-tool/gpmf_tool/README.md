...
**「フォルダから結合」** ボタンを押すとフォルダ選択 → 中身を自動で精査し、
Windows エクスプローラーの詳細表示に寄せた一覧で結果を見せる。

```
  結合  名前                        サイズ    長さ   解像度      撮影日時         種類
 ─────────────────────────────────────────────────────────────────────────────
  ☑    分割された1本の撮影 — 3個 / 12:04 / 5.31 GB            …          結合対象
...
- **サムネイル表示** — 埋め込みサムネ → 同名 `.THM`/`.jpg` → ffmpeg の順に取得
  （Pillow が入っていれば実画像、無ければ情報のみ）
- **取捨選択** — チェック欄のクリック / Space キーで含める・外すを切替。
  グループ行のチェックでまとめて切替。「すべて選択 / 解除」も可
- **列** — 名前・サイズ・長さ・解像度・撮影日時・種類（360度 / GoPro / GPS）
- ダブルクリックで OS の既定プレイヤーで再生して中身を確認できる
- 下部に選択中の合計サイズが出るので、必要な空き容量が事前にわかる
- **「結合する」** / **「結合して GoPro 化」** のどちらかを選んで実行
