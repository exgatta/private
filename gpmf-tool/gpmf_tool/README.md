...
```bash
# 並び順に結合
python3 -m gpmf_tool join part1.mp4 part2.mp4 part3.mp4 -o 結合.mp4

# フォルダ内の分割ファイルをまとめて (名前から自動グループ化)
python3 -m gpmf_tool join ./videos --auto-group --output-dir ./out
```
...
- GUI では **「分割動画を結合 (DJI等)」** ボタン → 動画を複数選択 → 保存先を指定

> 結合してから GoPro 化すると、テレメトリも 1 本につながって扱いやすい。
